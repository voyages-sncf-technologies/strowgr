 ...
 */
public class CorrectedCachingAuthenticator<C, P> implements Authenticator<C, User> {
    private final Authenticator<C, User>          underlying;
    private final Cache<C, Optional<User>> cache;
    ...
    public CorrectedCachingAuthenticator(MetricRegistry metricRegistry,
                                Authenticator<C, User> authenticator,
    ...
    @Override
    public Optional<User> authenticate(C credentials) throws AuthenticationException {
        final Timer.Context context = gets.time();
        try {
            Optional<User> optionalPrincipal = cache.getIfPresent(credentials);
