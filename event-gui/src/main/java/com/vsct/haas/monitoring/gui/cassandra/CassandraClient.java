...
public class CassandraClient {
    ...
    static SortedSetMultimap<Key, Value> result = TreeMultimap.create(new Comparator<Key>() {
        @Override
    ...
    public static void main(String[] args) throws ExecutionException, InterruptedException {
        ...
        ListenableFuture<SortedSetMultimap<Key, Value>> futur = Futures.transform(
                resultSetFuture,
                iterate(result)
        );

        futur.get().keys().forEach(k -> {
            System.out.print(k.correlationId + " ");
            result.get(k).forEach(v -> {
                if (showPayload) {
                    System.out.print(v.name + "|"+v.payload+"| ->");
                }
                else {
        ...
        System.out.println("Processed "+messageProcessed+" messages. Map contains "+result.size()+" keys.");
    }

    private static AsyncFunction<ResultSet, SortedSetMultimap<Key, Value>> iterate(SortedSetMultimap<Key, Value> result) {
        return new AsyncFunction<ResultSet, SortedSetMultimap<Key, Value>>() {
            @Override
            public ListenableFuture<SortedSetMultimap<Key, Value>> apply(ResultSet rs) throws Exception {
                ...
                for (Row row : rs) {
                    ...
                    Key k = new Key();k.timestamp=d;k.correlationId=correlationId;
                    Value v = new Value();v.name=name;v.timestamp=d;
